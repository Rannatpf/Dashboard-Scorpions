...
# Estilização CSS para cartões de métricas
st.markdown("""
    <style>
...
def carregar_e_tratar_dados():
    try:
        ...
        # Identifica colunas de meses (Ex: jan./2025 até jan. 2026)
        cols_temporais = [c for c in df.columns if '/' in str(c) or '202' in str(c)]
        
        # Converte para numérico garantindo que erros virem zero
        cols_numericas = cols_temporais + ['valor_mensal']
        for col in cols_numericas:
            if col in df.columns:
                df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0)

        # MÉTRICA DE NORMALIZAÇÃO: Resolve o erro de clientes que entraram depois
        # Conta quantos meses o cliente teve atividade > 0
        df['meses_ativos'] = df[cols_temporais].gt(0).sum(axis=1)
        df['meses_ativos'] = df['meses_ativos'].replace(0, 1) # Evita divisão por zero

        # Calcula Total e Média Mensal (Engajamento Real)
        df['total_mensagens'] = df[cols_temporais].sum(axis=1)
        df['media_mensal'] = df['total_mensagens'] / df['meses_ativos']

        # Trata Canais e Personas
        ...
        # Novo Status de Risco baseado na MÉDIA MENSAL (mais justo para novos clientes)
        # Consideramos risco se a média for menor que 500 mensagens/mês
        df['status_risco'] = df.apply(
            lambda x: 'Crítico' if x['media_mensal'] < 500 and x['status'] == 'ativo' 
...
if df is not None:
    ...
    st.markdown(f"Análise normalizada de **{len(df_filtrado)}** contas (Jan/25 - Jan/26).")
    
    m1, m2, m3, m4 = st.columns(4)
    m1.metric("Receita (MRR) Ativa", f"R$ {df_filtrado[df_filtrado['status']=='ativo']['valor_mensal'].sum():,.2f}")
    m2.metric("Média de Mensagens/Mês", f"{df_filtrado['media_mensal'].mean():,.0f}")
    ...
    m4.metric("Alertas de Risco (Média)", criticos)
    ...
    with tab_eng:
        c1, c2 = st.columns(2)
        with c1:
            st.subheader("Média Mensal por Persona")
            fig = px.bar(df_filtrado, x='persona', y='media_mensal', color='status', 
                         barmode='group', template="plotly_dark",
                         color_discrete_map={'ativo': '#00CC96', 'cancelado': '#EF553B'})
            st.plotly_chart(fig, use_container_width=True)
        with c2:
            st.subheader("Distribuição de Uso por Nicho")
            fig_nicho = px.box(df_filtrado, x='nicho', y='media_mensal', color='persona')
    ...
    with tab_fin:
        ...
        with c4:
            st.subheader("Correlação: Valor vs Média de Uso")
            fig_scat = px.scatter(df_filtrado, x='media_mensal', y='valor_mensal', 
                                 size='media_mensal', color='status', hover_name='cliente')
    ...
    with tab_risco:
        st.subheader("🧠 Centro de Monitoramento Pró-Ativo (Normalizado)")
        risco_df = df_filtrado[df_filtrado['status_risco'] == 'Crítico'].sort_values('media_mensal')
        
        st.error(f"Identificados {len(risco_df)} clientes ativos com média abaixo de 500 msgs/mês.")
        st.dataframe(risco_df[['cliente', 'nicho', 'media_mensal', 'total_mensagens', 'meses_ativos', 'valor_mensal']], 
